...
def plot_profile(data: pd.DataFrame, absolute=False, **kwargs):
    '''
    Plot a profile
    ...
    data.iloc[:, 0].pipe(micompanyify, ax=axes[0], ascending=None, plottype='bar', **kwargs)
    
    axes[1].set_title(name1)
    data.iloc[:, 1].fillna(0).pipe(micompanyify, ax=axes[1], ascending=None, plottype='bar', **kwargs)
    if absolute:
        outcome = data.iloc[:, 1].sub(data.iloc[:, 0], fill_value=0)
    else:
        outcome = data.iloc[:, 1].div(data.iloc[:, 0], fill_value=0)
    outcome.pipe(micompanyify, ax=axes[2], ascending=None, plottype='bar', strfmt='.1f' if absolute else '.1%', **kwargs)
...
def define_colors(highlight, data, plottype):
    ...
    if isinstance(data, pd.Series) or plottype == 'scatter':
        ...
        if plottype == 'line_timeseries':
            ...
            n = data.shape[1]
            color = [cmap(x) for x in np.linspace(1/n, 1, n)]
    else:
    ...
    **kwargs: will be passed to pd.Dataframe.plot()
    ...
    if plottype == 'bar':
        ...
        if isinstance(data, pd.Series):
            plot_values_above_bar(data, orient=orient, strfmt=strfmt, ax=ax)
        else:
        ...
        plot_values_above_bar(location.iloc[highlight, :], data.iloc[highlight, :],
                              textcolor='white', orient=orient, strfmt=strfmt, ax=ax)
